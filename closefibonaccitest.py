...
def testfibonacci1():
    capOut = setUp()
    maths_fun.closefibonacci(5)
    assert ("5 is in the Fibonacci sequence") == capOut.getvalue().strip()
    tearDown()

def testfibonacci2():
    capOut = setUp()
    maths_fun.closefibonacci(9)
    assert ("9 is not in the Fibonacci sequence\nThe next larger number in the Fibonacci sequence is 13") == capOut.getvalue().strip()
    tearDown()
